...
DEFINE VARIABLE bmiktar AS DECIMAL FORMAT "->>>>9.999" NO-UNDO INITIAL 0.
...
DEFINE INPUT PARAMETER TABLE FOR xkayit_table.
...
REPEAT:
    ...
    DO TRANSACTION ON ERROR UNDO, RETURN ERROR:
        ...
        FIND FIRST xkayit_table NO-LOCK NO-ERROR. /*hangi kayitlar degistiyse onlar guncellenecek malzeme guncelleme sorun oluyorda*/

        IF NOT AVAILABLE xkayit_table THEN DO:
            MESSAGE "Islem turu bilinmiyor sistem hatasi... Ýþ Emri No   : " xop_table.isemri_no "Ýstasyon Kod : " xop_table.istasyon_kod VIEW-AS ALERT-BOX TITLE "U y a r ý...".
            RETURN ERROR.
        END.
        ...
        FOR EACH xop_table ON ERROR UNDO, RETURN ERROR:
            ...
            IF NOT AVAILABLE is_istasyon THEN NEXT.
                ...
                IF NOT AVAILABLE erp_master THEN NEXT.
                ...
                IF xkayit_table.Malzemeler THEN DO:
				
					FOR EACH erp_recete WHERE erp_recete.firma_kod = firma-kod 
						AND erp_recete.erp_masterno = erp_master.erp_masterno AND erp_recete.operasyon_kod = xop_table.operasyon_kod ON ERROR UNDO, RETURN ERROR:
						
						FIND FIRST Malzemeler WHERE Malzemeler.MalzemeKod EQ erp_recete.stok_kod NO-LOCK NO-ERROR.
						...
						IF NOT AVAILABLE Malzemeler THEN DO:
							...
							IF stok_kart.int_sira NE 2 THEN DO:
								...
								RETURN ERROR.
							END.                        
							/*DISPLAY "Silindi Op:" + erp_recete.operasyon_kod + ", Stok:" + erp_recete.stok_kod NO-LABEL.*/
							DELETE erp_recete.
						END.
						/*ELSE DO:                        
							IF xop_table.miktar > 0 AND Malzemeler.MiktarSekli = 1 THEN DO:
                                DEFINE VARIABLE dBMiktar AS DECIMAL FORMAT "->>>>9.99999" NO-UNDO INITIAL 0.
                                DEFINE VARIABLE sBMiktar AS CHARACTER FORMAT "X(20)" NO-UNDO INITIAL "".
                                DEFINE VARIABLE iBMiktar AS INTEGER FORMAT "9999999999" NO-UNDO INITIAL 0.
                                dBMiktar = Malzemeler.KMiktar / xop_table.miktar.
                                sBMiktar = STRING(dBMiktar).
                                iBMiktar = INTEGER(sBMiktar).
                                sBMiktar = STRING(iBMiktar) + "." + SUBSTRING(sBMiktar, INDEX(sBMiktar, ".") + 1, 5).
                                dBMiktar = DECIMAL(sBMiktar).
								IF erp_recete.miktar NE dBMiktar THEN DO:
									ASSIGN
										erp_recete.miktar = dBMiktar
										erp_recete.dbirim = Malzemeler.Birim
										erp_recete.aciklama = "Mikrobar tarafindan degistirildi.".
								END.
							END.
						END.*/ /*END ELSE DO*/
					END.
                ...
                FIND erp_detay2 WHERE erp_detay2.firma_kod = firma-kod AND erp_detay2.kayit_id = xop_table.kayit_no EXCLUSIVE-LOCK NO-ERROR NO-WAIT.

                IF  NOT AVAILABLE erp_detay2 THEN DO:       
                    MESSAGE "Uretim bilgisinde hata var! " xop_table.kayit_no "(erp_detay2)" VIEW-AS ALERT-BOX TITLE "Hata!".        
                    RETURN ERROR.
                END.
                ...
                ASSIGN           
                    ...
                    erp_detay2.sira_no        = erp_detay.sira_no 
                     erp_detay2.aciklama1      = "Mikrobar Tarafýndan Guncellendi."
                    ...
                    END.
                     ...
                     IF xop_table.bit_saati NE "" AND xop_table.bas_saati NE "" THEN DO:
                         ...
                         IF psure EQ ? THEN psure = 0.                              
                         IF psure < 0 THEN psure = 0.                
                         IF psure EQ ? THEN psure = 0.                           
                     END. /* ne*/
                     ...
                     IF xkayit_table.Iscilikler THEN {INCLUDE3\UretimKaydiGuncelle_Iscilik.i}.
                     IF xkayit_table.Duruslar THEN {INCLUDE3\UretimKaydiGuncelle_Durus.i}.
                     IF xkayit_table.Aletler THEN {INCLUDE3\UretimKaydiGuncelle_Alet.i}
                     IF xkayit_table.Hurdalar THEN {INCLUDE3\UretimKaydiGuncelle_Hurda.i}                      
            ...
            END. 
            RELEASE erp_detay2.
            IF xkayit_table.Malzemeler THEN RUN INCLUDE3\UretimKaydiGuncelle_Malzeme.i (INPUT x5recid, INPUT TABLE Malzemeler).
